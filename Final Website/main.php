        ...
        <div class="col-sm-12">
            ...
            <div class="row placeholders">
                ...
                <div class="col-xs-12 col-sm-12 placeholder">
                    <h2><br/>Today's progress for each website compared to your set preferences<br/></h2>
                ...
                <h2><br/>Overall Website Use For This Date<br/></h2>
                <div class="dropdown" style="text-align:center;height:50px;">
                ...
                <div class="col-xs-12 col-sm-12 placeholder">
                    <h2 style="height:50px;"><br/>Overall Website Use For The Last 5 Days<br/></h2>
                    <canvas id="lineChart1"></canvas>
                    <span class="text-muted">Daily representation of separate websites compared to daily targets<br/></span>
		    <span class="text-muted">Straight lines represent prefered (for positive) or maximum (for negative) time that has been previously indicated by you</span>
                </div>
            </div>

	    <h2 class="sub-header"><br/><span style="font-size: 25px" class="glyphicon glyphicon-list"></span> Overall Prefered Time for Selected Websites</h2>
            <div class="table-responsive">
                <table id="preferedTable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th>Website</th>
                        <th>Prefered Time</th>
            ...
            <h2 class="sub-header"><br/><span style="font-size: 25px" class="glyphicon glyphicon-list"></span> Individual Daily Statistics</h2>
            <div class="table-responsive">
...
<script>
    ...
    $(document).ready(function(){
		...
		var dailyTimeVar = (new Date).clearTime().addSeconds(userWebsites[i].preferedTime).toString('H:mm:ss');
		if(userWebsites[i].website != 'other') {
		    $('<tr><td>' + userWebsites[i].website + '</td><td>' + dailyTimeVar + '</td><td>' + userWebsites[i].webType + '</td></tr>').appendTo('.tableClass');
                }
		preferedTime.push(userWebsites[i].preferedTime);
            ...
            for(var i = 0; i < allWebsites.length; i++) {
                if(i == 0) {
		    if(allWebsites[0] == 'other') document.getElementById('websiteDropdown1').firstChild.data = allWebsites[1];
		    else document.getElementById('websiteDropdown1').firstChild.data = allWebsites[i];
		}
	    ...
	    var tempIndex = 0;
	    if (allWebsites[tempIndex] == 'other') tempIndex = 1;

            for(var i = 0; i < newData[0][4].length; i++) {
                if(newData[0][4][i].website == allWebsites[tempIndex]) {
                    progressResult = newData[0][4][i].dailyTime * 100 / preferedTime[tempIndex];
                }
            }
            if(progressResult > 100) progressResult = 100;
        
            if(webType[tempIndex] == 'positive') progressType = 'progress-bar-success';
